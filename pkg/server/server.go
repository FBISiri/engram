...
func (s *Server) registerTools() {
	...
	addTool := mcp.NewTool("memory_add",
		...
		mcp.WithNumber("importance", mcp.Description("Importance score. Per-type defaults if omitted: identity=6, directive=7, insight=5, event=4. Per-type bounds (values outside are clamped): identity [5,9], directive [5,10], insight [3,8], event [1,7]. Omit to use per-type default.")),
