> 归档：2026-07-17 之前的中文版 README，英文版重写后归档于此。
...
**最小启动配置（OpenAI embedder）：**

```bash
export ENGRAM_QDRANT_URL=localhost:6334        # Qdrant gRPC (默认值)
export ENGRAM_OPENAI_API_KEY=sk-...            # 必填
./engram serve
```

**切换 Voyage AI embedder：**
...
**完整配置项** → [`docs/configuration.md`](docs/configuration.md)（覆盖 Qdrant、embedding、评分权重、去重阈值、Reflection Engine、OTel、TTL、多 collection）
